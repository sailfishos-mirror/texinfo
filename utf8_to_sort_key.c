 ...
 */
int
decode_utf8_char (const unsigned char *str, size_t len, char32_t *codepoint)
{
  if (len == 0)
    {
      return -1;
...
UTF8Result
utf8_to_codepoints (const char *utf8_str)
{
  ...
  result.codepoints =
    (char32_t *) malloc (num_codepoints * sizeof (char32_t));
