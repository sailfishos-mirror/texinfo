  ...
  if (label_target_name_sv)
    {
      ...
      if (label_element)
        label_element_sv = SvREFCNT_inc ((SV *) label_element->sv);
      else
  ...
  if (node_file_name_sv)
    {
      ...
      *called = 1;
      ...
      PUSHs(sv_2mortal (SvREFCNT_inc ((SV *) self->sv)));
      PUSHs(sv_2mortal (SvREFCNT_inc ((SV *) target_element->sv)));
  ...
  if (sectioning_command_target_name_sv)
    {
      ...
      PUSHs(sv_2mortal (SvREFCNT_inc ((SV *) self->sv)));
      PUSHs(sv_2mortal (SvREFCNT_inc ((SV *) command->sv)));
      PUSHs(sv_2mortal (newSVpv (target, 0)));
  ...
  if (external_target_split_name_sv)
    {
      ...
      PUSHs(sv_2mortal (SvREFCNT_inc ((SV *) self->sv)));
      PUSHs(sv_2mortal (newSVpv (normalized, 0)));
      PUSHs(sv_2mortal (SvREFCNT_inc ((SV *) element->sv)));
      PUSHs(sv_2mortal (newSVpv (target, 0)));
                                  ...
                                  const ELEMENT *element, const char *target)
{
  ...
  if (element)
    element_sv = SvREFCNT_inc ((SV *) element->sv);
  else
    element_sv = newSV (0);
                              ...
                              const char *filename)
{
  ...
  if (command)
    command_sv = SvREFCNT_inc ((SV *) command->sv);
  else
    command_sv = newSV (0);
      ...
      if (arg_formatted->arg_tree)
        {
          ...
          hv_store (arg_formated_hv, "arg_tree", strlen ("arg_tree"),
                    SvREFCNT_inc ((SV *) arg_formatted->arg_tree->sv), 0);
