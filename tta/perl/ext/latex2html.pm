...
EOT
  ...
  if (!close (L2H_LATEX)) {
    $self->converter_document_warn(sprintf(__(
      "l2h: error on closing latex file %s: %s"), $l2h_latex_path_name, $!));
  }
  return 1;
}
...
sub l2h_process($$)
{
  ...
  if ($destination_directory ne '') {
    ...
    $l2h_latex_path_name = $l2h_latex_file_name;
  }
  ...
  if ($destination_directory ne '') {
    ...
    $l2h_cache_path_name = "${docu_name}-l2h_cache.pm";
  }
...
# FIXME is it really necessary? Check if there is a real need for the renaming.
# In addition, if there is some SRC="(.*?)" matched in the text some errors
# could ensue.
...
sub l2h_change_image_file_names($$)
{
  ...
  foreach my $src (@images) {
    ...
    unless (defined($dest)) {
      ...
      my $src_file;
      if ($destination_directory ne '') {
        $src_file = join('/', ($destination_directory, $src));
      } else {
        $src_file = $src;
      }
      ...
      my $file_dest;
      if ($destination_directory ne '') {
        $file_dest = join('/', ($destination_directory, $dest));
      } else {
        $file_dest = $dest;
      }
...
sub l2h_retrieve_from_html($$)
{
  ...
  my $l2h_html_path_name;
  if ($destination_directory ne '') {
    $l2h_html_path_name = "$destination_directory/$l2h_html_file_name";
  } else {
    $l2h_html_path_name = $l2h_html_file_name;
  }
  ...
  if (!close(L2H_HTML)) {
    $self->converter_document_warn(
                sprintf(__("l2h: error on closing html file %s: %s"),
                                 $l2h_html_path_name, $!));
  }
  return @html_retrieved_text_indices;
}
...
sub l2h_convert_command($$$;$$)
{
  ...
  # The default formatting would lead to a warning message since the
...
#
# Cache latex2html output as it may take a long time to run compared to
# texi2any.

# It was originally tried with a dbm data base, but it did not store all
# keys/values. Hence, do as latex2html does using Perl code to store the
# cache as code defining the %l2h_cache hash keys and values.
...
sub l2h_store_cache($)
{
  ...
  if (!close(FH)) {
    $self->converter_document_warn(sprintf(__(
      "l2h: error on closing cache file %s: %s"), $l2h_cache_path_name, $!));
  }
}
