...
sub _convert($$)
{
  ...
  while ($next = $reader->read()) {
    ...
    if ($e_type and $ignored_types{$e_type}) {
...

    if ($e_type and $category == Texinfo::Reader::TXI_ELEMENT_START) {
    ...
    if ($e_type and $category == Texinfo::Reader::TXI_ELEMENT_END) {
      if (defined($type_elements{$e_type})) {
        $$output_ref .= "</$type_elements{$e_type}>";
      } elsif ($e_type eq 'preformatted') {
        $$output_ref .= "</$self->{'document_context'}->[-1]->{'preformatted_stack'}->[-1]>";
