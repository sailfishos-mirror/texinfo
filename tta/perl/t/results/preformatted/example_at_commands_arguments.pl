...
$result_converted_errors{'html'}->{'example_at_commands_arguments'} = '* W |must specify a title with a title command or @top
...
$result_converted{'latex_text'}->{'example_at_commands_arguments'} = '\\begin{document}
