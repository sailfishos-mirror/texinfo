...
/* ALTIMP perl/Texinfo/Convert/Info.pm */
...
MODULE = Texinfo::Convert::Info	PACKAGE = Texinfo::Convert::Info
...
converter (SV *class, SV *conf_sv=0)
      ...
      CODE:
        ...
        self = converter_converter (COF_info, user_conf);
        if (user_conf)
...
output (SV *converter_in, SV *document_in)
      ...
      CODE:
        document = get_converter_and_document_from_sv (converter_in,
                                                     document_in, &self);
        result = info_output (self, document);
