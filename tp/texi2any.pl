...
."\n"
.sprintf(__("  %s --html --no-headers foo.texi  write html without node lines, menus"), $real_command_name)."\n"
.sprintf(__("  %s --number-sections foo.texi    write Info with numbered sections"), $real_command_name)."\n"
.sprintf(__("  %s --no-split foo.texi           write one Info file however big"), $real_command_name)."\n"
."\n";
