...
int collation_element_is_variable (struct collation_data *element);

struct collation_data_ref {
  const struct collation_data *array;
...
void get_implicit_weight (uint32_t codepoint, struct collation_data *elements,
                          size_t *n_elements);

void print_collation (FILE *stream, const struct collation_data *elements,
                      size_t num_elements);
