...
%
\def\texinfoversion{2026-09-06.15}
%
...
\def\txiwarning#1{%
  \message{^^J\linenumber Warning: #1^^J}%
}
...
\def\badparencount{%
  \txiwarning{unbalanced parentheses in @def...}%
  \global\parencount=0
}
\def\badbrackcount{%
  \txiwarning{unbalanced square brackets in @def...}%
  \global\brackcount=0
}
...
\def\macroxxx#1{%
  ...
  \if1\csname ismacro.\the\macname\endcsname
     \txiwarning{redefining \the\macname}%
  \else
...
\parseargdef\listoffloats{%
  ...
  \expandafter\ifx\csname floatlist\safefloattype\endcsname \relax
    \ifhavexrefs
      ...
      \txiwarning{\No `\safefloattype' floats to list.}%
    \fi
  \else
    \begingroup
...
\def\documentencodingzzz#1{%
  ...
  \ifxetex
    ...
    \else
      \ifx \declaredencoding \ascii
      \else
        \txiwarning{XeTeX with non-UTF-8 encodings cannot handle %
...
\gdef\UTFviiiDefined#1{%
  \ifx #1\relax
    \ifutfviiidefinedwarning
      \txiwarning{Unicode char \string #1 not defined for Texinfo}%
    \fi
  \else
    \expandafter #1%
  \fi
