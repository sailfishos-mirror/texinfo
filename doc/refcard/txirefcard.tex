...
% along with Texinfo.  If not, see <https://www.gnu.org/licenses/>.
%
...
\def\versionyear{2024}
\def\versionTexinfo{7.2.92}
...
\txicmd{@documentlanguage}{ll\rmbrack[\code{_}cc]}{Declare the current language as the two-character \acro{ISO}-639 abbreviation \var{ll}, optionally followed by an underscore and a two-letter \acro{ISO} 3166 two-letter country code (\var{cc}).}
...
\txicmd{@documentinfo}{}{Paired with \code{@end documentinfo}.  Start block containing \code{@title}, \code{@subtitle} or \code{@author} lines.}
\txicmd{@title}{title}{Define the title for the title page.}
\txicmd{@subtitle}{subtitle}{Define the subtitle for the title page.}
\txicmd{@author}{author}{Define an author for the title page.}
\txicmd{@publication}{}{Contains information such as publisher address and ISBN.  Pair with \code{@end publication}.}
\txicmd{@maketitle}{}{Output a title page and possibly the following copyright page using information from \code{@documentinfo}, \code{@copying}, and \code{@publication}.}

\txicmd{@shorttitlepage}{title}{Generate a minimal title page, generally for the first page of output in a long printed manual.}
\txicmd{@titlepage}{}{Begin a title page with explicit formatting. Write the command on a line of its own, paired with \code{@end titlepage}. For printed manuals only in the default case.}
...
\txicmd{@namedanchor}{\ttbraced{\var{name}, \var{xref-name}}}{Define \var{name} as the current location, using \var{xrefname} as a label for any cross-references.}
...
\txicmd{@xrefname}{text}{Provide text for referencing a node, without printing a heading.}
...
\txicmd{@thischapter, @thischaptername, @thischapternum, @thissection, @thissectionname, @thissectionnum, @thispart, @thisfile, @thispage, @thistitle}{}{These commands are only allowed in a heading or footing. They stand for, respectively, the number and name of the current chapter (in the format `Chapter~1: Title'), the current chapter name only, the current chapter number only, the number and name of the current section, the current section name only, the current section number only, the name of the current part, the filename, the current page number, and the title of the document.}
